...
SELECT FLOOR(AVG(length/60)) + ROUND(AVG((length/60)%2), 2) AS average_duration
FROM film;

SELECT length%60 AS average_duration
FROM film;
...
-- 2.1 Calculate the number of days that the company has been operating.rental
...
SELECT rating, COUNT(*) AS number_film_rating
FROM film
GROUP BY rating;
...
FROM film
GROUP BY rating
ORDER BY number_film_rating DESC;
